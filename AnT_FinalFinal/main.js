...
let globalPhase = -1; // -1: Title, 0: Heart, 1: Star, 2: Blackhole
let phase = "title";
let switchFrame = -1;
let t = 0;
let BGCOLOR = "#010713";
...
let creditY = 0;
let creditTexts = [
  "석준혁 : 프로세싱을 통해 이렇게 추상적인 주제로 짧은 미디어 아트를 만들 수 있어서 감명 깊었다. 특히 AI를 통해 코딩에 있어서 큰 도움을 얻을 수 있어서 좋았다.",
  "조현빈 : 처음에 이 프로젝트를 시작할때에는 우리가 과연 1분 가까이 되는 미디어 아트를 만들어낼 수 있을까 하는 의문이 들었다. 그러나 우리가 만들고자 하는 작품의 방향성을 정확하게 수립한 후, AI에게 학습을 시켜 우리가 원하고자 하는 작품을 출력하게 한다면, 이정도 수준의 작품을 만드는 데는 어려움이 없는 수준까지 AI가 발전했다는 것을 다시금 느낄 수 있었다.",
  "최효우 : 우리가 이러한 AI를 잘 사용하려면, 전반적인 코드의 흐름을 파악하고, 그 코드를 활용할 수 있는 힘이 있을때 AI를 올바르게 사용할 수 있다는 점을 깨닫는 계기가 되었다.",
  "AI 사용 비율은 75% 입니다.",
  "preload(): 별의 이미지, 중성자별의 이미지 등 리소스를 미리 불러오기 위해 사용하였습니다.",
  "setup(): 캔버스 초기화 및 변수 설정을 위해 사용하였습니다.",
  "이 외에도 CoreParticle, SupernovaParticle, Shockwave 등을 활용해 기능과 효과들을 객체로 구현하였습니다. 또한 stage 변수로 인터렉트, 초신성, 잔해 단계를 구분하였습니다. 마지막으로 이미지를 불러오기 위한 preload() 와 stage 변수로 단계 구분을 하였습니다.",
  "",
  "감사합니다!"
];
...
function preload() {
  ...
  finalHeartImg = loadImage('f5zhlg8oaqzpqfcctux8.png');
...
function drawTitleScreen() {
  ...
  if (titleTimer > 60) {
    titleAlpha = min(titleAlpha + 2, 255);
  }
  ...
  textSize(72);
  text("기억의 재구성", width / 2, height / 2 - 60);
  
  // 부제목
  textSize(36);
  fill(200, 200, 200, titleAlpha * 0.8);
  text("당신은 본인의 기억을 믿나요?", width / 2, height / 2 + 40);
  
  // 시작 안내
  if (titleTimer > 180) {
    let blinkAlpha = titleAlpha * (sin(frameCount * 0.1) * 0.5 + 0.5);
    fill(150, 150, 150, blinkAlpha);
    textSize(24);
    text("클릭하여 시작", width / 2, height / 2 + 120);
  }
}
...
function drawBlackholeMain() {
  ...
  switch (state.phase) {
    ...
    case "ending":    drawEndingCredits(); break;
  }
  drawDebugInfo();
}
...
function drawEndingCredits() {
  ...
  let currentY = creditY;
  
  for (let i = 0; i < creditTexts.length; i++) {
    if (i === creditTexts.length - 1) {
      // 마지막 "감사합니다!" 텍스트
      fill(255, 255, 100);
      textSize(64);
    } else if (creditTexts[i] === "") {
      // 빈 줄
      currentY += 40;
      continue;
    } else {
    ...
    // 텍스트가 너무 길면 여러 줄로 분할
    let lines = splitTextToLines(creditTexts[i], width - 100);
    for (let line of lines) {
      if (currentY > -50 && currentY < height + 50) {
        text(line, width / 2, currentY);
      }
      currentY += (i === creditTexts.length - 1) ? 80 : 40;
    }
    
    currentY += 20; // 항목 간 여백
  }
  
  // 크레딧이 모두 지나가면 처음으로 돌아가거나 종료
  if (creditY < -2000) {
    // 원하면 여기서 다시 타이틀로 돌아가거나 종료할 수 있음
  }
}
...
function updateAndDrawDust() {
  ...
  for (let d of dustParticles) {
    ...
    let grav = toCenter.copy().setMag(map(distToCenter, 0, width / 2, 0.07 * 4, 0.45 * 4/650));
    d.vel.add(grav);
    d.vel.mult(0.97);
    d.pos.add(d.vel);
    fill(d.baseColor);
